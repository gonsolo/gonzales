...
extension MixMaterial {
        ...
        static func create(parameters: ParameterDictionary, textures: [String: Texture], namedMaterials: [String: UninstancedMaterial], state: inout State) throws -> MixMaterial {
                ...
                guard let materials = try? parameters.findStrings(name: "materials"), materials.count == 2 else {
                        ...
                        return MixMaterial(material1: defaultMat, material2: defaultMat, amount: Texture.floatTexture(amountIdx))
                }
                ...
                if textureName != "", let tex = textures[textureName] {
                        ...
                        let idx = state.arena.appendFloat(FloatTexture.constantTexture(ConstantTexture<Real>(value: scalarAmount)))
                        amount = Texture.floatTexture(idx)
                } else {
                        let idx = state.arena.appendFloat(FloatTexture.constantTexture(ConstantTexture<Real>(value: 0.5)))
                        amount = Texture.floatTexture(idx)
                }
