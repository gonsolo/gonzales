struct PointLight: Light {
        ...
        func sample(for reference: Interaction, u: Point2F) -> (radiance: Spectrum, direction: Vector, pdf: FloatX, visibility: Visibility) {
                ...
                let pdf: FloatX = 1.0
                let visibility = Visibility(from: reference, to: SurfaceInteraction(position: position))
...
func createPointLight(lightToWorld: Transform, parameters: ParameterDictionary) throws -> PointLight {
        guard let intensity = try parameters.findSpectrum(name: "I") else { throw ParameterError.missing(parameter: "I") }
        guard let scale = try parameters.findSpectrum(name: "scale", else: white) else { throw ParameterError.missing(parameter: "scale") }
