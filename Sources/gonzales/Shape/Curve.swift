...
fileprivate func lerp(with t: FloatX, between first: Point, and second: Point) -> Point {
...
final class Curve: Shape {
        ...
        func objectBound() -> Bounds3f {
                ...
                let bounds = Union(first: Bounds3f(first: points.0, second: points.1),
        ...
        private func subdivideBezier(points: [Point]) -> [Point] {
                ...
                splitPoints.append((points[0] + Point(3 * points[1]) + Point(3 * points[2]) + points[3]) / 8)
        ...
        func evalBezier(points: FourPoints, u: FloatX) -> (Point, Vector) {
                let cp1 = ThreePoints(lerp(with: u, between: points.0, and: points.1),
                                      lerp(with: u, between: points.1, and: points.2),
                                      lerp(with: u, between: points.2, and: points.3))
                let cp2 = TwoPoints(lerp(with: u, between: cp1.0, and: cp1.1),
        ...
        func recursiveIntersect(ray: Ray,
                                tHit: inout FloatX,
                                points: [Point],
                                index: Int = 0,
                                rayToObject: Transform,
                                u: TwoFloats,
                                depth: Int) throws -> (SurfaceInteraction?, FloatX) {
                ...
                if depth > 0 {
                        ...
                        var hits: [InteractionAndT] = [(nil, 0), (nil, 0)]
                        for segment in 0..<2 {
                                let cps = segment * 3
                                let maxWidth = computeMaxWidth(u: (u[segment], u[segment+1]), width: common.width)
                                if !overlap(points: splitPoints, index: cps, xyz: 1, width: maxWidth) { continue }
                                if !overlap(points: splitPoints, index: cps, xyz: 0, width: maxWidth) { continue }
                                let zMax = rayLength * tHit
                                if !overlap(points: splitPoints, index: cps, xyz: 2, width: maxWidth, limits: (0, zMax)) { continue }
                                hits[segment] = try recursiveIntersect(ray: ray,
                                                                       tHit: &tHit,
                                                                       points: splitPoints,
                                                                       index: cps,
                        ...
                        if hits[0].0 != nil && hits[1].0 != nil {
                                if hits[0].1  < hits[1].1 {
                                        return hits[0]
                                } else {
                                        return hits[1]
                                }
                        } else if hits[0].0 != nil && hits[1].0 == nil {
                                return hits[0]
                        } else if hits[0].0 == nil && hits[1].0 != nil {
                                return hits[1]
                        } else {
                        ...
                        func testTangent(_ a: Int, _ b: Int) -> Bool {
                                let edge = (points[a].y - points[b].y) * -points[b].y + points[b].x * (points[b].x - points[a].x)
                                if edge < 0 {
                                        return false
                                } else {
                                        return true
                        ...
                        let nu = clamp(value: lerp(with: w, between: u.0, and: u.1), low: u.0, high: u.1)
                        let hitWidth = lerp(with: nu, between: common.width.0, and: common.width.1)
                        let (pc, dpcdw) = evalBezier(points: points, u: clamp(value: w, low: 0, high: 1))
                        ...
                        let dpdvPlane = normalized(Vector(x: -dpduPlane.y, y: dpduPlane.x, z: 0)) * hitWidth
        ...
        private func computeWidth(u: TwoFloats, width: TwoFloats) -> (FloatX, FloatX) {
                let result = (lerp(with: u.0, between: width.0, and: width.1),
                              lerp(with: u.1, between: width.0, and: width.1))
                return result
        }

        private func overlap(points: [Point], index: Int = 0, xyz: Int, width: FloatX, limits: TwoFloats = (0, 0)) -> Bool {
                let pMax = max(max(points[index+0][xyz], points[index+1][xyz]),
                               max(points[index+2][xyz], points[index+3][xyz]))
                let pMin = min(min(points[index+0][xyz], points[index+1][xyz]),
        ...
        func intersect(ray worldRay: Ray, tHit: inout FloatX) throws -> SurfaceInteraction? {
                ...
                let objectToRay = try lookAtTransform(eye: ray.origin, at: ray.origin + ray.direction, up: dx)
                ...
                var l0: FloatX = 0
                for i in 0..<2 {
                        let px: FloatX = abs(controlPoints[i].x - 2.0 * controlPoints[i+1].x + controlPoints[i+2].x)
                        let py: FloatX = abs(controlPoints[i].y - 2.0 * controlPoints[i+1].y + controlPoints[i+2].y)
                        let pz: FloatX = abs(controlPoints[i].z - 2.0 * controlPoints[i+1].z + controlPoints[i+2].z)
                ...
                let interactionAndT = try recursiveIntersect(ray: ray,
                                                             tHit: &tHit,
...
struct CurveCommon {
        ...
        init(points: FourPoints, width: TwoFloats) {
                self.points = points
                self.width = width
        }
...
func createCurveShape(objectToWorld: Transform, parameters: ParameterDictionary) throws -> [Shape] {
        ...
        for segment in 0..<numberOfSegments {
                ...
                bezierPoints.2 = p233
                bezierPoints.3 = p333
                let w0 = lerp(with: FloatX(segment) / FloatX(numberOfSegments), between: width0, and: width1)
                let w1 = lerp(with: FloatX(segment + 1) / FloatX(numberOfSegments), between: width0, and: width1)
                let curve = createCurve(objectToWorld: objectToWorld,
                                        points: bezierPoints,
                                        width: (w0, w1))
