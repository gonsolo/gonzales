...
protocol Integrator {
        ...
        func getRadianceAndAlbedo(from ray: Ray, tHit: inout FloatX,
                for scene: Scene, with sampler: Sampler) throws
                -> (radiance: Spectrum, albedo: Spectrum, normal: Normal)
}

extension Integrator {

        private func chooseLight(inScene scene: Scene,
                                 withSampler sampler: Sampler) throws
                -> (Light, FloatX) {

                assert(scene.lights.count > 0)
                guard scene.lights.count + scene.infiniteLights.count > 0 else { throw RenderError.noLights }
                let u = sampler.get1D()
        ...
        func sampleOneLight(at interaction: SurfaceInteraction,
                            bsdf: BSDF,
                            with sampler: Sampler,
                            in scene: Scene) throws -> Spectrum {

                guard scene.lights.count > 0 else { return black }
                let (light, lightPdf) = try chooseLight(inScene: scene, withSampler: sampler)
                let estimate = try estimateDirect(light: light,
        ...
        private func estimateDirect(light: Light,
                            atInteraction interaction: SurfaceInteraction,
                            bsdf: BSDF,
                            withSampler sampler: Sampler) throws -> Spectrum {

                let zero = (black, FloatX(0.0), up)

                func sampleLightSource() throws -> (estimate: Spectrum, density: FloatX, sample: Vector) {
                        let (radiance, wi, lightDensity, visibility) = light.sample(for: interaction, u: sampler.get2D())
                        guard try !radiance.isBlack && !lightDensity.isInfinite && visibility.unoccluded() else {
                                return zero
                        }
                        ...
                        //print("scatter \(scatter) radiance \(radiance) reflected \(reflected)")
                        return (estimate: estimate, density: lightDensity, sample: wi)
                }

                func lightDensity(sample: Vector) throws -> FloatX {
                        return try light.probabilityDensityFor(samplingDirection: sample, from: interaction)
                }

                func sampleBrdf() throws -> (estimate: Spectrum, density: FloatX, sample: Vector) {
                        var (scatter, wi, bsdfDensity, _) = try bsdf.sample(wo: interaction.wo, u: sampler.get2D())
                        ...
                        guard let brdfInteraction = try scene.intersect(ray: ray, tHit: &tHit) else {
                        ...
                        let radiance = brdfAreaLight.emittedRadiance(from: brdfInteraction,
                                                                     inDirection: -wi)
                ...
                let lightSampler = MultipleImportanceSampler<Vector>.Sampler(sample: sampleLightSource, density: lightDensity)
                let brdfSampler = MultipleImportanceSampler<Vector>.Sampler(sample: sampleBrdf, density: brdfDensity)
        ...
        func russianRoulette(beta: inout Spectrum) -> Bool {
                ...
                if roulette < probability { return true }
                else { beta /= probability; return false }
        }

        func intersectOrInfiniteLights(ray: Ray, tHit: inout FloatX, bounce: Int, L: inout Spectrum) throws -> SurfaceInteraction? {
                if let interaction = try scene.intersect(ray: ray, tHit: &tHit) { return interaction }
                let radiance = scene.infiniteLights.reduce(black, { accumulated, light in accumulated + light.radianceFromInfinity(for: ray) })
