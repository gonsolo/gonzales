struct FresnelSpecular: BxDF {
        ...
        private func reflective(wo: Vector, fresnel: FloatX) -> (RGBSpectrum, Vector, FloatX) {
                let wi = Vector(x: -wo.x, y: -wo.y, z: wo.z)
                let radiance = fresnel * reflectance * absCosTheta(wi)
                let pdf = fresnel
                return (radiance, wi, pdf)
        }

        private func transmittive(wo: Vector, fresnel: FloatX) -> (RGBSpectrum, Vector, FloatX) {
                var etaI = etaA
                var etaT = etaB
                ...
                let pdf = 1 - fresnel
                let radiance = transmittance * (1 - fresnel)
