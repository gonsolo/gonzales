...
template<typename T>
inline __both__ T dot(const vec3<T> &a, const vec3<T> &b)
{
	return a.x*b.x + a.y*b.y + a.z*b.z;
}

template<typename T>
inline __both__ vec3<T> normalize(const vec3<T> &v)
{
	return v * 1.f / sqrt(dot(v,v));
}

template<typename T>
inline __both__ vec3<T> cross(const vec3<T> &a, const vec3<T> &b) {
	return vec3<T>(
		a.y*b.z - b.y*a.z,
                a.z*b.x - b.z*a.x,
                a.x*b.y - b.x*a.y);
