...
func sphericalDirection(sinTheta: FloatX, cosTheta: FloatX, phi: FloatX, x: Vector, y: Vector, z: Vector) -> Vector {
...
func Union(bound: Bounds3f, point: Point) -> Bounds3f {
        let pMin = Point(x: min(bound.pMin.x, point.x),
                           y: min(bound.pMin.y, point.y),
                           z: min(bound.pMin.z, point.z))
        let pMax = Point(x: max(bound.pMax.x, point.x),
